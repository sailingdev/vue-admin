<template lang="html">
  ...
  <section class="special-feature-detail">
    ...
    是否包邮：买卖双方自行商议</textarea>
            ...
            <div class="col-lg-10 offset-1">
                <div class="form-row" style="margin-bottom: 25px">
                    <label for="device" class="col-form-label"><b>推送设备：</b></label>
                    <div class="col-lg-6 justify-content-between" style="margin-left: 20px">
                        <b-dropdown class="col-lg-4 dropdown-selector text-input" text="自然" variant="outline-secondary">
            ...
            <b-modal ref="modalspecialfeature" size="lg" centered hide-footer>
              ...
              <div class="button-panel row" style="margin-left: 20px">
                <div class="search-box">
                    ...
                    <b-form-input class="search-input" type="text" placeholder="查询特辑名称" v-model="special_feature_name" @input="change_special_feature"></b-form-input>
                </div>
                <div class="row">
                    <span class="normal-label">开始时间：</span>
                    <datepicker placeholder="请选择开始时间" format="yyyy-MM-dd" v-model="special_feature_start_time"></datepicker>
                </div>&nbsp;&nbsp;&nbsp;
                <div class="row">
                    <span class="normal-label">结束时间：</span>
                    <datepicker class="my-datepicker" placeholder="请选择结束时间" format="yyyy-MM-dd" v-model="special_feature_end_time"></datepicker>
                </div>
                <div style="position: absolute; right: 30px">
                    <b-button variant="dark" class="btn-rounded"><span style="font-size: 16px" @click="special_feature_search">查找</span></b-button>&nbsp;
                    <b-button variant="secondary" class="btn-rounded"><span style="font-size: 16px" @click="special_feature_clear">清空</span></b-button>
                </div>
              </div>
              ...
              <div class="table-responsive" style="padding-top: 20px;">
                <table id="special_feature_table" class="table center-aligned-table" style="text-align: center">
                    ...
                    <tbody>
                    <tr v-for="special_feature in temp_special_features" :key="special_feature.id">
...
export default {
  ...
  data () {
    return {
      ...
      special_feature_start_time: '',
      special_feature_end_time: '',
      items: [
        {
          id: '45685229878',
  ...
  methods: {
    change_special_feature () {
      ...
      if (this.special_feature_name === '') {
        this.special_features.forEach(item => this.users.push(item))
      } else {
        this.special_features.forEach(item => {
          if (item.id.includes(this.special_feature_name) || item.owner.includes(this.special_feature_name)) {
            this.temp_special_features.push(item)
          }
        })
      }
    },
    special_feature_search () {
      var month = ''
      var date = ''
      var startDate = ''
      var endDate = ''
      if (this.special_feature_start_time !== '') {
        if (this.special_feature_start_time.getMonth() + 1 < 10) {
          month = '0' + (this.special_feature_start_time.getMonth() + 1)
        } else {
          month = this.special_feature_start_time.getMonth() + 1
        }
        if (this.special_feature_start_time.getDate() < 10) {
          date = '0' + this.special_feature_start_time.getDate()
        } else {
          date = this.special_feature_start_time.getDate()
        }
        startDate = this.special_feature_start_time.getFullYear() + '-' + month + '-' + date
      }
      if (this.special_feature_end_time !== '') {
        if (this.special_feature_end_time.getMonth() + 1 < 10) {
          month = '0' + (this.special_feature_end_time.getMonth() + 1)
        } else {
          month = this.special_feature_end_time.getMonth() + 1
        }
        if (this.special_feature_end_time.getDate() < 10) {
          date = '0' + this.special_feature_end_time.getDate()
        } else {
          date = this.special_feature_end_time.getDate()
        }
        endDate = this.special_feature_end_time.getFullYear() + '-' + month + '-' + date
      }
      this.temp_special_features = []
    ...
    special_feature_clear () {
      this.special_feature_start_time = ''
      this.special_feature_end_time = ''
